...
my $apr_version = catfile $ap_dir{include}, 'apr_version.h';
open(my $inc, $apr_version)
    or die "Cannot open $apr_version: $!";
my %vers;
while (<$inc>) {
    if (/define\s+APR_(MAJOR|MINOR|PATCH)_VERSION\s+(\d+)/) {
        $vers{$1} = $2;
    }
}
close $inc;
...
closedir $dir;
die "Unable to find the apr lib" unless ($apr_libname = $libs[0]);

my %apr_args = (APR_MAJOR_VERSION => $vers{MAJOR},
                APR_DOTTED_VERSION => $dotted,
...
foreach my $var (keys %apr_args) {
    print $fh qq{my \${$var} = q[$apr_args{$var}];\n};
}
